...
grayA1 = glmmTMB(MaxN ~ sand, family = compois(), data = df_gray_A) #only good one
...
grayA_sandpreds = ggpredict(grayAF7, terms = c("sand [0:100 by = 5]")) |> 
  rename(sand = x, MaxN = predicted)
  
grayA_mpapreds = ggpredict(grayAF7, terms = c("mpa_distance [0:500 by = 1]")) |>
  rename(mpa_distance = x, MaxN = predicted)

grayA_patchpreds = ggpredict(grayAF7, terms = c("pland_AggregatedPatchReefs [0:100 by = 5]")) |>
  rename(pland_AggregatedPatchReefs = x, MaxN = predicted)
  
grayA_rugopreds = ggpredict(grayAF7, terms = c("Avg_rugosity [1:2.5 by = 0.05]")) |>
...
grayA_mpa_preds = ggplot(grayA_mpapreds, aes(x = mpa_distance, y = MaxN)) +  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill =
 "#66C2A5", alpha = 0.3) +
  geom_line( linewidth = 1) +
  theme_classic() +
...
grayA_apr_preds = ggplot(grayA_patchpreds, aes(x = pland_AggregatedPatchReefs, y = MaxN)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill =
 "#66C2A5", alpha = 0.3) +
  geom_line( linewidth = 1) +
  theme_classic() +
  labs(title = "Predicted Graysby Abundance (MaxN) by cover of Aggregated Patch Reef",
...
ggarrange(gray_sand_pred, grayA_mpa_preds, grayA_apr_preds, gray_rugo_pred,
          labels = c('a)', 'b)', 'c)','D)'),
          ncol = 2, vjust = 2, align = "v")
...
redAF1 = glmmTMB(MaxN ~ pland_Seagrass + (1|Site), data = df_red_A, family = poisson())
redAF2 = glmmTMB(MaxN ~ pland_Seagrass + (1|Site), data = df_red_A, family = nbinom2())
redAF3 = glmmTMB(MaxN ~ pland_Seagrass + (1|Site), data = df_red_A, family = compois())
redAF4 = glmmTMB(MaxN ~ pland_Seagrass , data = df_red_A, family = poisson())
redAF5 = glmmTMB(MaxN ~ pland_Seagrass , data = df_red_A, family = nbinom2())
redAF6 = glmmTMB(MaxN ~ pland_Seagrass , data = df_red_A, family = compois())

AICc(redAF1,redAF2,redAF3,redAF4,redAF5,redAF6)
...
yellowAF1 = glmmTMB(MaxN ~ sponge + pland_AggregateReef, data = df_yellow_A, family = poisson())
yellowAF2 = glmmTMB(MaxN ~ sponge + pland_AggregateReef + (1|Site), data = df_yellow_A, family = poisson())
yellowAF3 = glmmTMB(MaxN ~ sponge + pland_AggregateReef, data = df_yellow_A, family = nbinom2())
yellowAF4 = glmmTMB(MaxN ~ sponge + pland_AggregateReef + (1|Site), data = df_yellow_A, family = nbinom2())
yellowAF5 = glmmTMB(MaxN ~ sponge + pland_AggregateReef, data = df_yellow_A, family = compois())
yellowAF6 = glmmTMB(MaxN ~ sponge + pland_AggregateReef + (1|Site), data = df_yellow_A, family = compois())

AICc(yellowAF1, yellowAF2,yellowAF3,yellowAF4,yellowAF5,yellowAF6)
...
ggarrange(yellow_sponge_pred, yellow_pland_pred,
          labels = c('a)', 'b)', 'c)'),
          ncol = 2, vjust = 1, align = "v")
...
schoolAF1 = glmmTMB(MaxN ~ Rugosity_variance + (1|Site), data = df_school_A, family = poisson())
schoolAF2 = glmmTMB(MaxN ~ Rugosity_variance + (1|Site), data = df_school_A, family = nbinom2())
schoolAF3 = glmmTMB(MaxN ~ Rugosity_variance + (1|Site), data = df_school_A, family = compois())
schoolAF4 = glmmTMB(MaxN ~ Rugosity_variance, data = df_school_A, family = poisson())
schoolAF5 = glmmTMB(MaxN ~ Rugosity_variance, data = df_school_A, family = nbinom2())
schoolAF6 = glmmTMB(MaxN ~ Rugosity_variance, data = df_school_A, family = compois())

AICc(schoolAF1,schoolAF2,schoolAF3,schoolAF4,schoolAF5,schoolAF6)

schoolAF = glmmTMB(MaxN ~ Rugosity_variance, data = df_school_A, family = poisson())
check_model(schoolAF)

SimulationOutput1 <- simulateResiduals(fittedModel = schoolAF, 
                                        refit = FALSE, 
                                        seed = 12345)
testDispersion(SimulationOutput1, 
                type = "DHARMa",
